...
#ifdef CONFIG_AVNET5282
#define BIT_BANG_THE_MII
#endif
...
#  if defined(CONFIG_UC5272_PHY_INT)
 ...
 #define ICR_PHY_SETUP	0x00c00000
...
#define PHY_INT			MCF_INT_INT2
  ...
  #define ICR_PHY_SETUP	0x0c000000
 ...
 **********/
#define FEC_DEBUG 1
	// higher numbers give more debug 7 is highest so far ...MaTed---
...
// ----------------------------------------------------------------
#ifdef BIT_BANG_THE_MII
static uint bit_bang_hack = 0xffffffff;

// Per the ColdFire data sheet, MDIO is port AS bit 5 and MDC is port AS bit 4.
// However, on the Avnet ColdFire 5282 board, these were connected reversed.
// If they were connected correctly, we could have the FEC serialize and
// deserialize via its MDATA and MSCR registers.  But instead we configure
// those two pins as GPIO and bit-bang them directly.

#define MDIO_BIT    4 // See above comment
#define MDC_BIT     5 // See above comment

#define MDC_HI     (1 << MDC_BIT)
#define MDIO_HI    (1 << MDIO_BIT)

#define MCF5282_GPIO_DDRAS      (*(volatile unsigned char  *)0x40100020)
#define MCF5282_GPIO_CLRAS      (*(volatile unsigned char  *)0x40100048)
#define MCF5282_GPIO_PORTASP    (*(volatile unsigned char  *)0x40100034)
#define MCF5282_GPIO_PASPAR     (*(volatile unsigned char  *)0x40100056)

// See above comment.  Configure port AS pins 5 and 4 for GPIO
// by clearing PASPAR[11-10] and [9-8], respectively.
void MII_bit_bang_initialize(void)
{
	MCF5282_GPIO_PASPAR &= ~0x0f00;
}

// ----------------------------------------------------------------
// CF GPIO semantics are weird:
// * To read bits, read ASP
// * To set   a bit, write ASP
// * To clear a bit, write CLRAS

static int write_clock_and_read_data(
	int clock_val)
{
	int rv;

	// Data direction register (DDR):  1 for output, 0 for input.
	MCF5282_GPIO_DDRAS &= ~MDIO_HI;   // We will read  the data
	MCF5282_GPIO_DDRAS |=  MDC_HI;    // We will write the clock

	// Read the data
	if (MCF5282_GPIO_PORTASP & MDIO_HI)
		rv = 1;
	else
		rv = 0;
	...
	// Write the clock
	if (clock_val == 0)
		MCF5282_GPIO_CLRAS   = ~MDC_HI; // Low  clock
	else
		MCF5282_GPIO_PORTASP =  MDC_HI; // High clock
...
// ----------------------------------------------------------------
static void write_clock_and_data(
	int clock_val,
	int data_val)
{
	// Data direction register (DDR):  1 for output, 0 for input.
	MCF5282_GPIO_DDRAS |= MDC_HI;     // We will write the clock
	MCF5282_GPIO_DDRAS |= MDIO_HI;    // We will write the data
	...
	// Write the data
	if (data_val == 0)
		MCF5282_GPIO_CLRAS   = ~MDIO_HI; // Low  data
	else
		MCF5282_GPIO_PORTASP =  MDIO_HI; // High data

	udelay(1);
}

static uint MII_bit_bang_read(uint regval)
{
	int  bit;
	int  i;
	uint result;
	...
	// Preamble:  Hold MDIO high for at least 32 MDC clocks (note that
	// we drive the clock).
	for (i = 0; i < 36; i++) {
		write_clock_and_data(0, 1);
		write_clock_and_data(1, 1);
	}

	// Setup:  Write 0, 1 to MDIO.
	write_clock_and_data(0, 0);
	write_clock_and_data(1, 0);
	...
	// 5 which-port bits
	for (bit = 4; bit >= 0; bit--) {
		write_clock_and_data(0, (id >> bit) & 1);
		write_clock_and_data(1, (id >> bit) & 1);
	}

	// 5 register bits
	for (bit = 4; bit >= 0; bit--) {
		write_clock_and_data(0, (addr >> bit) & 1);
		write_clock_and_data(1, (addr >> bit) & 1);
	}
...
static void MII_bit_bang_write(uint regval)
{
	uint id   = (regval >> 23) & 0x1f;
	uint addr = (regval >> 18) & 0x1f;
	...
	// Turn around
	(void)write_clock_and_data(0, 1);
	(void)write_clock_and_data(1, 1);
	(void)write_clock_and_data(0, 0);
	(void)write_clock_and_data(1, 0);

	// 16 data bits
	for (bit = 15; bit >= 0; bit--) {
		(void)write_clock_and_data(0, (data >> bit) & 1);
		(void)write_clock_and_data(1, (data >> bit) & 1);
	}
}

#endif // BIT_BANG_THE_MII
...
fec_enet_mii(struct net_device *dev)
{
	...
	if ((mip = mii_head) != NULL)
#ifdef BIT_BANG_THE_MII
		MII_bit_bang_write(mip->mii_regval);
		bit_bang_hack = MII_bit_bang_read(mip->mii_regval);
#else
...
#ifdef BIT_BANG_THE_MII
			MII_bit_bang_write(regval);
			bit_bang_hack = MII_bit_bang_read(regval);
#else
...
static void mii_display_config(struct net_device *dev)
{
	...
	sprintf(msgbuf, "%s: config: auto-negotiation ", dev->name);
	...
	if (*s & PHY_CONF_100FDX)
		strcat(msgbuf, ", 100FDX");
	if (*s & PHY_CONF_100HDX)
		strcat(msgbuf, ", 100HDX");
	if (*s & PHY_CONF_10FDX)
		strcat(msgbuf, ", 10FDX");
	if (*s & PHY_CONF_10HDX)
		strcat(msgbuf, ", 10HDX");
	if (!(*s & PHY_CONF_SPMASK))
		strcat(msgbuf, ", No speed/duplex selected?");

	if (*s & PHY_CONF_LOOP)
		strcat(msgbuf, ", loopback enabled");
	
	strcat(msgbuf, ".\n");
	printk(KERN_DEBUG "%s", msgbuf);
...
fec_enet_open(struct net_device *dev)
{
	...
	if (fep->phy) {
		...
		for (timeout = 0; timeout < 8000; timeout++) {
		    if (!fep->sequence_done) {
			schedule();
		    }
		}
		if (!fep->sequence_done) {
		    fec_enet_close(dev);
		    printk("%s: configuration failure (%d)\n",
			   dev->name, fep->sequence_done);
		    return -EBUSY;
		}
...
int __init fec_enet_init(struct net_device *dev)
{
	...
	*/
	...
	if (iap[0] == 0xff && iap[1] == 0xff && iap[2] == 0xff &&
	    iap[3] == 0xff && iap[4] == 0xff && iap[5] == 0xff)
		iap = eap;
#elif defined(CONFIG_BOARD_UC5272) && 0
	...
	*/
	...
	/* Setup pin multplexing for ethernet/MII lines */
	*(volatile unsigned char *)(MCF_IPSBAR + MCF5282_GPIO_PEHLPAR) |= 0xc0;
	*(volatile unsigned short *)(MCF_IPSBAR + MCF5282_GPIO_PASPAR) |= 0x0f00;
